...
Diaporama.prototype = assign({}, EventEmitter.prototype, {
  ...
  _loadMedia: function () {
    ...
    this._data.timeline.forEach(function (item) {
      if (item.video) {
        item.video.crossOrigin = true;
        item.video.setAttribute('webkit-playsinline', 'webkit-playsinline');
        item.video.setAttribute('playsinline', 'playsinline');
  ...
  _start: function () {
    ...
    this._curLoop = raf(function loop (t) {
      ...
      if (!last) {
        // First tick
        last = t;
        self.emit("play", self.currentTime);
      }
      var dt = t - last;
      last = t;
